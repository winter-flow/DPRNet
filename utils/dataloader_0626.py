...
class CODataset(data.Dataset):
    ...
    def __init__(self, image_root, gt_root, edge_root, trainsize, augmentations):
        ...
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.png') or f.endswith('.tif') or f.endswith('.jpg')]

        # 06.27日补充,此前训练self.edges使用的是gt_root,也就是说,此前训练并没有使用到edge的信息
        self.edges = [edge_root + f for f in os.listdir(edge_root) if f.endswith('.png') or f.endswith('.tif') or f.endswith('.jpg')]
...
class test_dataset:
    def __init__(self, image_root, gt_root, testsize):
        ...
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png') or f.endswith('.tif')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.tif') or f.endswith('.png') or f.endswith('.bmp') or f.endswith('.jpg')]
        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
